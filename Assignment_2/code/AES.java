...
//	System.out.println("ENC Block len : "+in.length);
byte[] tmp = new byte[in.length];
...
public static int[] encrypt_pixel(int[] in,byte[] key){
	...
	lenght = 4 - in.length % 4;    
	padding = new int[lenght];     
	padding[0] = (int) 0x80;
...
//	byte[] tmp = new byte[in.length + lenght];  
	...
	int[] tmpInt = new int[in.length + lenght];  
	int[] blocInt = new int[16];       
	...
	for (i = 0; i < in.length + lenght; i++) {
		 if (i > 0 && i % 4 == 0) {
			bloc = intToByteArray(blocInt);
			bloc = encryptBloc(Arrays.copyOf(bloc, 16));
			blocInt = byteArrayToIntArray(bloc);
		  	System.arraycopy(blocInt, 0, tmpInt, i - 4, blocInt.length);
		 }
		 if (i < in.length)
			 blocInt[i % 4] = in[i];
		 else	{              
			 blocInt[i % 4] = padding[count % 4];
		  	count++;
		 }
	}
...
public static int[] decrypt_pixel(int[] in,byte[] key){
	int i;
	int[] tmpInt = new int[in.length];
	byte[] bloc = new byte[16];
	byte[] tmp = new byte[16];
	
	int[] blocInt = new int[4];
//	int[] blocInt = null;
	Nb = 4;
	Nk = key.length/4;
	Nr = Nk + 6;
	...
	for (i = 0; i < in.length; i++) {
		if (i > 0 && i % 4 == 0) {
			...
			bloc = intToByteArray(blocInt);
			bloc = decryptBloc(Arrays.copyOf(bloc, 16));
			blocInt = byteArrayToIntArray(bloc);
		  	System.arraycopy(blocInt, 0, tmpInt, i - 4, blocInt.length);
	...
	tmp = intToByteArray(tmpInt);
	tmp = deletePadding(tmp);	
	tmpInt = byteArrayToIntArray(tmp);
...
public static int[] encrypt_pixel_r(int[] in,byte[] key){
...
public static int[] decrypt_pixel_r(int[] in,byte[] key){
	int i;
